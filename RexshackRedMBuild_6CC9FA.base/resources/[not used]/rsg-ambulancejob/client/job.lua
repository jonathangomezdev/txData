...
function createAmbuPrompts()
    for k, v in pairs(Config.Locations["armory"]) do
        exports['rsg-core']:createPrompt("ambulance:armory:"..k, vector3(v.x, v.y, v.z), Config.PromptKey, 'Armory', {
            type = 'client',
            event = 'ambulance:client:promptArmory',
        })
    end
    for k, v in pairs(Config.Locations["duty"]) do
        exports['rsg-core']:createPrompt("ambulance:duty:"..k, vector3(v.x, v.y, v.z), Config.PromptKey, 'On/Off Duty', {
            type = 'client',
            event = 'ambulance:client:promptDuty',
        })
    end
    for k, v in pairs(Config.Locations["vehicle"]) do
        exports['rsg-core']:createPrompt("ambulance:vehicle:"..k, vector3(v.x, v.y, v.z), Config.PromptKey, 'Jobgarage', {
            type = 'client',
    ...
    for k, v in pairs(Config.Locations["stash"]) do
        exports['rsg-core']:createPrompt("ambulance:stash:"..k, vector3(v.x, v.y, v.z), Config.PromptKey, 'Personal Stash', {
            type = 'client',
            event = 'ambulance:client:promptStash',
        })
    end
    for k, v in pairs(Config.Locations["checking"]) do
        exports['rsg-core']:createPrompt("ambulance:checkin:"..k, vector3(v.x, v.y, v.z), Config.PromptKey, 'Check-in', {
            type = 'client',
            event = 'ambulance:client:promptCheckin',
        })
    end
    for k, v in pairs(Config.Locations["beds"]) do
        exports['rsg-core']:createPrompt("ambulance:bed:"..k, vector3(Config.Locations["beds"][k].coords.x, Config.Locations["beds"][k].coords.y, Config.Locations["beds"][k].coords.z), Config.PromptKey, Lang:t('text.lie_bed', {cost = Config.BillCost}), {
            type = 'client',
...
RegisterNetEvent('ambulance:client:promptArmory', function()
    RSGCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerJob.name == "ambulance"  then
            ...
            RSGCore.Functions.Notify(Lang:t('error.not_ems'), 'error')
        end
    end)
end)
...
RegisterNetEvent('ambulance:client:promptStash', function(k)
    RSGCore.Functions.GetPlayerData(function(PlayerData)
        ...
        onDuty = PlayerData.job.onduty
        if PlayerJob.name == "ambulance"  then
            TriggerServerEvent("inventory:server:OpenInventory", "stash", "ambulancestash_"..RSGCore.Functions.GetPlayerData().citizenid)
            TriggerEvent("inventory:client:SetCurrentStash", "ambulancestash_"..RSGCore.Functions.GetPlayerData().citizenid)
        else
            RSGCore.Functions.Notify(Lang:t('error.not_ems'), 'error')
        end
    end)
...
RegisterNetEvent('hospital:client:CheckStatus', function()
    ...
    if player ~= -1 and distance < 5.0 then
        ...
        RSGCore.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
            if result then
                for k, v in pairs(result) do
                    ...
                    elseif result["WEAPONWOUNDS"] then
                        for k, v in pairs(result["WEAPONWOUNDS"]) do
                            TriggerEvent('chat:addMessage', {
                                color = { 255, 0, 0 },
                                multiline = false,
                                args = { Lang:t('info.status'), RSGCore.Shared.Weapons[v2].damagereason }
                            })                        end
                    elseif result["BLEED"] > 0 then
						...
						RSGCore.Functions.Notify(Lang:t('success.healthy_player'), 'success')
                    end
                end
...
RegisterNetEvent('hospital:client:RevivePlayer', function()
    ...
    if hasItem then
        ...
        if player ~= -1 and distance < 5.0 then
            ...
            count = 1
            RSGCore.Functions.Progressbar("reviving", "Reviving...", 5000, false, true, {
...
RegisterNetEvent('hospital:client:TreatWounds', function()
    ...
    if hasItem then
        ...
        if player ~= -1 and distance < 5.0 then
            ...
            FreezeEntityPosition(PlayerPedId(), true)
            count = 1
            RSGCore.Functions.Progressbar("reviving", "Treast Wounds...", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
