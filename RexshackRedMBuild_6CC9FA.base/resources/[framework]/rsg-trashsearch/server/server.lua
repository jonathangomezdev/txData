...
RegisterNetEvent('rsg-trashsearch:server:searchTrash', function(data)
    local src = source
    ...
    TriggerClientEvent('rsg-trashsearch:client:searchTrash', src, binId)
end)
...
RegisterNetEvent('rsg-trashsearch:server:searchedTrash', function(binId)
    local src = source

    -- Check if player is legit
    if not isLegit(src) then
        TriggerClientEvent("rsg-anticheat:client:ban", src, "rsg-trashsearch -> Player " .. GetPlayerName(src) .. " tried to bypass cooldown")
        error("rsg-trashsearch -> Player tried to bypass cooldown")
    end
