        ...
        if Races ~= nil then
                ...
                for k in pairs(Races) do
                    if Races[k] ~= nil then
                        if #(pos - vector3(Races[k].startx, Races[k].starty, Races[k].startz)) < 15.0 and not Races[k].started then
                            sleep = 0
                            DrawText3D(Races[k].startx, Races[k].starty, Races[k].startz, "[J] To join the race ($"..Races[k].amount..")")
            ...
            if ShowCountDown then
                if #(pos - vector3(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz)) < 15.0 and Races[RaceId].started then
                    sleep = 0
                    DrawText3D(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz, "Race start in "..RaceCount)
                end
            end
        end
        Wait(sleep)
