...
RegisterNetEvent('rsg-horsetrainer:server:updatexp',function(action)
    ...
    if action == 'leading' and horsexp <= Config.FullyTrained then
        ...
        if Config.TrainingEXPNotification then
			...
			TriggerEvent('rsg-log:server:CreateLog', 'horse', 'Leading horse ', 'orange', firstname..' '..lastname..'Is Leading a Horse named '..horsename..' XP IS NOW '..newxp..' 🐎')
        end
    ...
    if action == 'cleaning' and horsexp <= Config.FullyTrained then
        ...
        if Config.TrainingEXPNotification then
            TriggerClientEvent('RSGCore:Notify', src, horsename..'\'s'..Lang:t('success.xp_now')..newxp, 'success')
			TriggerEvent('rsg-log:server:CreateLog', 'horse', 'Cleaning horse ', 'orange', firstname..' '..lastname..' Cleaned Horse named '..horsename..' XP IS NOW '..newxp..' 🐎')
        end
    ...
    if action == 'feeding' and horsexp <= Config.FullyTrained then
        ...
        if Config.TrainingEXPNotification then
            TriggerClientEvent('RSGCore:Notify', src, horsename..'\'s'..Lang:t('success.xp_now')..newxp, 'success')
			TriggerEvent('rsg-log:server:CreateLog', 'horse', 'Feeding horse ', 'orange', firstname..' '..lastname..' Fed  Horse named '..horsename..' XP IS NOW '..newxp..' 🐎')
