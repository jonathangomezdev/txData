...
RegisterNetEvent('rsg-weapons:client:UseWeapon', function(weaponData, serial)
    local ped = PlayerPedId()
    ...
    if not alreadyUsed and not UsedWeapons[tonumber(hash)] then
		...
		wepSerial = tostring(weaponData.info.serie)
...
RegisterNetEvent('rsg-weapons:client:AddAmmo', function(ammotype, amount, ammo)
	...
	print('^5Max Ammo^7	: '..tostring(max_ammo))
	print('^5Loaded^7		: '..tostring(amount))
	print('^5Total^7		: '..tostring(total))
...
CreateThread(function()
    while true do
        ...
        if currentSerial ~= nil and heldWeapon ~= -1569615261 and not switchingWeapon then
