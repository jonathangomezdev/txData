Config = {}

Config.Debug = true

-- settings
Config.UpdateAmmo = 30000 -- amount of time before saving player ammo in miliseconds

-- limit the amount of ammo players can load
Config.MaxArrowAmmo = 16
Config.MaxRevolverAmmo = 100
Config.MaxPistolAmmo = 100
Config.MaxRepeaterAmmo = 100
Config.MaxRifleAmmo = 100
Config.MaxVarmintAmmo = 100
Config.MaxShotgunAmmo = 100

-- amount of ammo per load
Config.AmountArrowAmmo = 16
Config.AmountRevolverAmmo = 100
Config.AmountPistolAmmo = 100
Config.AmountRepeaterAmmo = 100
Config.AmountRifleAmmo = 100
Config.AmountVarmintAmmo = 100
Config.AmountShotgunAmmo = 100
Config.AmountThrowablesAmmo = 10


--- Damage Weapon
Config.WeaponDmg = 0.65
Config.MeleeDmg = 1.0

Config.AmmoTypes = {
    ['weapon_thrown_throwing_knives'] = `AMMO_THROWING_KNIVES`,
    ['weapon_thrown_tomahawk'] = `AMMO_TOMAHAWK`,
    ['weapon_thrown_tomahawk_ancient'] = `AMMO_TOMAHAWK_ANCIENT`,
    ['weapon_thrown_bolas'] = `AMMO_BOLAS`,
    ['weapon_thrown_bolas_hawkmoth'] = `AMMO_BOLAS_HAWKMOTH`,
    ['weapon_thrown_bolas_ironspiked'] = `AMMO_BOLAS_IRONSPIKED`,
    ['weapon_thrown_bolas_intertwined']  = `AMMO_BOLAS_INTERTWINED`,
    ['weapon_thrown_dynamite'] = `AMMO_DYNAMITE`,
    ['weapon_thrown_molotov'] = `AMMO_MOLOTOV`,
    ['weapon_thrown_poisonbottle'] = `AMMO_POISONBOTTLE`
}

