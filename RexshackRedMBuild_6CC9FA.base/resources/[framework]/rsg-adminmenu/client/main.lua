...
local menuLocation = 'center'
local menuSize = 'size-175'
local mainMenu = MenuV:CreateMenu(false, Lang:t('menu.admin_menu'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv')
local adminOptions = MenuV:CreateMenu(false, Lang:t('menu.admin_options'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv')
local playerOptions = MenuV:CreateMenu(false, Lang:t('menu.online_players'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv')
local serverOptions = MenuV:CreateMenu(false, Lang:t('menu.manage_server'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv')
local developerOptions = MenuV:CreateMenu(false, Lang:t('menu.developer_options'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv')
...
adminOptions:On('open', function(menu)
  ...
  menu:AddButton({
    ...
    select = function()
      TriggerServerEvent('rsg-logs:server:CreateLog', 'admin', 'Admin Options', 'red', GetPlayerName() .. ' toggled > INVISIBILITY <')
      if Invisible then
...
playerOptions:On('open', function(menu)
  menu:ClearItems()
  RSGCore.Functions.TriggerCallback('admin:server:getplayers', function(players)
...
serverOptions:On('open', function(menu)
  menu:ClearItems()
  RSGCore.Functions.TriggerCallback('admin:server:hasperms', function(hasperms)
    if hasperms then
      ...
      menuSlider1:On('select', function(item, value)
        TriggerServerEvent("rsg-weathersync:server:setWeather", value)
      end)

      menuSlider2:On('select', function(item, value)
        TriggerServerEvent("rsg-weathersync:server:setTime", value, value)
      end)
    end
  end, 'time')
end)
...
OpenPlayerMenu = function(player)
  local playerMenu = MenuV:CreateMenu(false, player.id .. Lang:t('info.options'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv')
...
OpenBanMenu = function(banplayer)
  local banMenu = MenuV:CreateMenu(false, Lang:t('menu.ban'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv', 'test9')
  banMenu:ClearItems()
...
OpenKickMenu = function(kickplayer)
  local kickMenu = MenuV:CreateMenu(false, Lang:t('menu.kick'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv', 'test7')
  kickMenu:ClearItems()
...
OpenPermsMenu = function(permsplayer)
  RSGCore.Functions.TriggerCallback('admin:server:hasperms', function(hasperms)
    if hasperms then
      local permMenu = MenuV:CreateMenu(false, Lang:t('menu.perms'), menuLocation, 220, 20, 60, menuSize, 'rsg-core', 'menuv', 'test8')
      permMenu:ClearItems()
      ...
      permMenu:AddButton({
        ...
        select = function(btn)
          if selectedgroup ~= 'Unknown' then
            ...
            RSGCore.Functions.Notify(Lang:t('success.changed_perms'), 'success')
...
GotoCoords = function(coords)
  if type(coords) ~= 'vector3' then
    RSGCore.Functions.Notify(Lang:t('error.invalid_coords'), 'error')
  end
