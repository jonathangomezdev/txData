...
RegisterNetEvent('rsg-weaponsmith:client:mainmenu', function(jobaccess)
    if currentjob == jobaccess then
        exports['rsg-menu']:openMenu({
            ...
            {
                header              = "Weapon Customisation Request",
                txt                 = "",
                icon                = "fas fa-tools",
                params =
                {
                    event           = 'rsg-weaponsmith:client:CheckCustRequest',
                    isServer        = false
                }
            },
            {
...
RegisterNetEvent('rsg-weaponsmith:client:CustRequest', function(requests)
    local CustRequest = {}
    CustRequest =
    {
        {
    ...
    for i = 1, #requests do
        ...
        local citizenid = req.citizenid
        local serial = req.serial
        local name = req.name
        local price = req.price

        CustRequest[#CustRequest + 1] =
        {
            header              = '('..citizenid..') '..name,
            txt                 = 'Serial: '..serial..'<br>'..'Price: $'..price,
            icon                = 'fas fa-cog',
            params =
            {
                event           = 'rsg-weaponsmith:server:ApproveCustRequest',
                isServer        = true,
                args =
                {
                    serial      = serial,
    ...
    exports['rsg-menu']:openMenu(CustRequest)
end)

-- Check Weapon Customisation Request
RegisterNetEvent('rsg-weaponsmith:client:CheckCustRequest', function()
    RSGCore.Functions.TriggerCallback('rsg-weaponsmith:server:CheckCustRequest', function(requests)
        if not requests then
            RSGCore.Functions.Notify('Ooops...', 'No weapon customisation requests available at the moment!', 'error', 3000)

            return
        end

        TriggerEvent('rsg-weaponsmith:client:CustRequest', requests)
    end)
end)
