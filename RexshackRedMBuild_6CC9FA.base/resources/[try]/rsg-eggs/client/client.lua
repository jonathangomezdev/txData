    ...
    while true do
        ...
        if bush then
            if active == false then
                local PlantgroupName  = CreateVarString(10, 'LITERAL_STRING', "Egg")
...
function goCollect()
    ...
    Wait(2300)
    TriggerServerEvent('qbr-eggs:addHerbs')
    active = false
...
function GetClosestBush()
    ...
    if size > 0 then
        for index = 0, size - 1 do
            ...
            if (model_hash ==  -2055098811 or model_hash ==  -1110548311) and not oldBush[tostring(entity)] then
