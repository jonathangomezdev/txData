...
RegisterNetEvent('rsg-oilsearch:server:searchTrash', function(data)
    local src = source
    ...
    TriggerClientEvent('rsg-oilsearch:client:searchTrash', src, binId)
end)
...
RegisterNetEvent('rsg-oilsearch:server:searchedTrash', function(binId)
    local src = source

    -- Check if player is legit
    if not isLegit(src) then
        TriggerClientEvent("rsg-anticheat:client:ban", src, "rsg-oilsearch -> Player " .. GetPlayerName(src) .. " tried to bypass cooldown")
        error("rsg-oilsearch -> Player tried to bypass cooldown")
    end
