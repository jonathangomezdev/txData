...
CreateThread(function()
	...
	for k,v in pairs(Config.JobPosters) do
		exports['rsg-core']:createPrompt(v.name, v.coords, 0xF3830D8E, 'Open ' .. v.name, {
			type = 'client',
			event = 'syn_jobboard:client_getPoster',
			args = {v.city},
		})
	end
...
RegisterNetEvent('syn_jobboard:client_getPoster', function(b)
	--print('args ' .. json.encode(args))
	--local _city = args.city
	--local a = table.unpack({"Velantine"})
	--print(a)
	local _city = b
	TriggerServerEvent("syn_jobboard:get_jobposters", _city)
...
RegisterNetEvent('syn_jobboard:send_list')
AddEventHandler('syn_jobboard:send_list', function(table,x,y)
	poster_table = table
	charidentifier = x
--[[ 	if y == "admin" then 
		group = true 
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.JobPosters) do
		...
		Citizen.InvokeNative(0x9CB1A1623062F402, blips, "Job Board")
	end
end)
...
RegisterNUICallback('send_to_poster', function(data, cb)
    for k,v in pairs(Config.JobPosters) do
		...
		if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.coords[1], v.coords[2], v.coords[3], false) < 10.0 then
			if data.Title ~= "" then
				local s = data.Link
				TriggerEvent("vorp:TipBottom", "You pinned a paper to the job board", 4000)
				TriggerServerEvent("syn_jobboard:set_link", data.Title, s:gsub("%'", ""), v.city)
				Citizen.Wait(1000)
				TriggerServerEvent("syn_jobboard:get_jobposters", v.city)
...
RegisterNUICallback('removepin', function(data, cb)
	...
	active = false
	cb('ok')
	TriggerServerEvent("syn_jobboard:removeposter", data.id)
end)
