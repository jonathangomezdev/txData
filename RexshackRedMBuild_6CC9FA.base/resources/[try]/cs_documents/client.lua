...
RegisterNetEvent('documents:menu', function( data )
    if data.menu == 'normal' then
        ...
        exports['rsg-menu']:openMenu(Docs)
...
AddEventHandler('documents:menu2', function( test )
    ...
    PlayerData = RSGCore.Functions.GetPlayerData()
    RSGCore.Functions.TriggerCallback('documents:getPlayerDetails', function (cb_player_details)
...
AddEventHandler('documents:showgive', function( data )
    ...
    elseif data.me == 'delete' then
        ...
        RSGCore.Functions.TriggerCallback('documents:deleteDocument', function (cb)
            if cb == true then
...
function GetAllUserForms()
    RSGCore.Functions.TriggerCallback('documents:getPlayerDocuments', function (cb_forms)
...
function OpenFormPropertiesMenu(cb_form)
    -- Código para mostrar las propiedades del formulario
end

RegisterNUICallback('form_submit', function(data, cb)
    RSGCore.Functions.TriggerCallback('documents:submitDocument', function (cb_form)
        if cb_form ~= nil then
            table.insert(USER_DOCUMENTS, cb_form)
            OpenFormPropertiesMenu(cb_form)
        else
            print ("Received nil from newly created scale object.")
        end
    end, data)
    SetNuiFocus(false, false)
end)
...
function GetNeareastPlayers()
    ...
    local players, nearbyPlayer = RSGCore.Functions.GetPlayersFromCoords()
